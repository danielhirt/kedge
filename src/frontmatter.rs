...
use crate::models::{DocFile, KedgeFrontmatter};

/// Raw YAML representation of the full frontmatter, used to extract the `kedge` sub-key.
#[derive(Debug, Deserialize)]
struct RawFrontmatter {
    kedge: Option<KedgeFrontmatter>,
}
...
/// Parse a markdown string and return a `DocFile` if it contains a `kedge:` block with anchors.
///
...
/// Read a markdown file and return a `DocFile` if it contains a `kedge:` block with anchors.
...
pub fn update_provenance_batch(
    ...
    let anchors = root
        ...
        .ok_or_else(|| anyhow::anyhow!("no kedge.anchors in {}", path.display()))?;
    ...
    let new_yaml =
        serde_yaml::to_string(&root).context("failed to serialize updated frontmatter")?;
