...
pub fn apply_classifications(
    ...
    let class_map: HashMap<(&str, Option<&str>), Severity> = classifications
        .iter()
    ...
    let drifted: Vec<TriagedDoc> = drift_report
        ...
        .map(|drifted_doc| {
            ...
            let doc_severity = triaged_anchors
                .iter()
                .map(|a| a.severity)
                .max()
...
pub async fn triage_drift_report(
    ...
    for drifted_doc in &drift_report.drifted {
        ...
        let response = provider::classify(triage_config, &prompt)
            .await
            .with_context(|| format!("triage call failed for doc {}", drifted_doc.doc))?;

        let classifications = parse_triage_response(&response).with_context(|| {
            format!(
                "failed to parse triage response for doc {}",
                drifted_doc.doc
            )
        })?;
